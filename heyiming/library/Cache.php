...
class Cache
{
    ...
    public static function init()
    {
        if (is_null(self::$instance)) {
            ...
            self::$instance->connect($GLOBALS['config']['redis']['host'], $GLOBALS['config']['redis']['port'], $GLOBALS['config']['redis']['timeout']);

            // 验证
            if ($GLOBALS['config']['redis']['pass'])
            {
                if ( !self::$instance->auth($GLOBALS['config']['redis']['pass']) ) 
                {
            ...
            self::$instance->setOption(Redis::OPT_PREFIX, $GLOBALS['config']['redis']['prefix'] . ":");
        }
    ...
    {
        if (is_null($expire)) {
            $expire = $GLOBALS['config']['redis']['expire'];
        }
        $key = $name;
